...
function parseResult(type, str, callback) {
  if (type === 'json') {
    var parsed;
    try {
  ...
  if (type === 'xml') {
    return require('xml2js').parseString(str, callback);
  }
...
module.exports = function (options, callback) {
  ...
  req.on('response', function (res) {
    ...
    res.on('end', function () {
      ...
      parseResult(responseType, data, function(err, parseResult) {
        callback(err, res, parseResult);
      });
    });    
  });
};
